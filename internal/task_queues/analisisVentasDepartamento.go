...
func AnalisisDepartamentoQueueTask(
	...
	f.SetCellValue(sheet, "D1", "Diferencia")
	f.SetCellValue(sheet, "E1", "Subtotal")
	f.SetCellValue(sheet, "F1", "Cantidad")
	f.SetCellValue(sheet, "G1", "Utilidad")
	f.SetCellValue(sheet, "H1", "NCosto")
	f.SetCellValue(sheet, "I1", "Precio")
	f.SetCellValue(sheet, "J1", "Costo")
	f.SetCellValue(sheet, "K1", "UtilidadPer")
	f.SetCellValue(sheet, "L1", "Total")
	...
	for idata, data := range d {
		...
		f.SetCellValue(sheet, fmt.Sprintf("D%d", row), data.Diferencia)
		f.SetCellValue(sheet, fmt.Sprintf("E%d", row), data.Subtotal)
		f.SetCellValue(sheet, fmt.Sprintf("F%d", row), data.Cantidad)
		f.SetCellValue(sheet, fmt.Sprintf("G%d", row), data.Utilidad)
		f.SetCellValue(sheet, fmt.Sprintf("H%d", row), data.NCosto)
		f.SetCellValue(sheet, fmt.Sprintf("I%d", row), data.Precio)
		f.SetCellValue(sheet, fmt.Sprintf("J%d", row), data.Costo)
		f.SetCellValue(sheet, fmt.Sprintf("K%d", row), data.UtilidadPer)
		f.SetCellValue(sheet, fmt.Sprintf("L%d", row), data.Total)
