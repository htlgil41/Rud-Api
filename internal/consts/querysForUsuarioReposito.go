...
var (
	...
	QUERY_PREPARE_UPDATE_USUARIO_REPOPG = `
		UPDATE SET usuarios SET username = $1, email = $2, departamento = $3 WHERE id = $4
	`
	QUERY_PREPARE_DELETE_USUARIO_REPOPG = `
		DELETE FROM usuarios WHERE id = $1
	`
	QUERY_PREPARE_ACT_USUARIO_REPOPG = `
		UPDATE SET usuarios SET activo = $1 WHERE id = $2
	`
	QUERY_PREPARE_ACT_PASSWORD_REPOPG = `
		UPDATE SET usuarios SET password_hash = $1 WHERE id = $2
	`
